  ...
  <div style="width: 100%" align="center">
    <div style="width: 1401px">
      <el-form :inline="true" :model="search" class="demo-form-inline" ref="search" style="float: left">
        <el-form-item label="销售单编号" prop="sid">
          <el-input v-model="search.sid" placeholder="销售单编号"></el-input>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" icon="el-icon-search" @click="searchSales">搜索</el-button>
          <el-button type="primary" style="width: 89px" @click="resetForm('search')">重置</el-button>
        </el-form-item>
      </el-form>

      <div class="clear"></div>
      <el-table :data="sales" border style="width: 1101px" :key="tableKey" >
        <el-table-column align="center" sortable  prop="sid" label="销售单编号" width="150px"></el-table-column> <!-- prop与下面的集合里的数据对应起来 -->
        ...
        <el-table-column align="center" sortable  prop="price" label="单价" width="120px">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" sortable  prop="tel" label="会员电话" width="120px"></el-table-column>
...
export default {
  methods: {
    ...
    searchSales() {
      const _this = this;
      axios.get('http://localhost:8181/api/sales/getsales/1', {params: this.search}).then(function (resp) {
        if (resp.data.code === 200) {
          _this.sales = resp.data.data.content
    ...
    page(currentpage){
      const _this = this;
      axios.get('http://localhost:8181/api/sales/getsales/'+currentpage,{params: this.search}).then(function (resp) {
        _this.sales = resp.data.data.content
        _this.total = resp.data.data.totalElements
        _this.lastElement = (resp.data.data.last && resp.data.data.numberOfElements === 1)
      })
    ...
    Delete(row, index){
      this.$confirm('您确定要删除吗?', '提示', {
        ...
        const _this = this
        axios.get('http://localhost:8181/api/sales/deletesales', {params: row}).then(function (resp) {
          if (resp.data.code === 200) {
            _this.$message.success(resp.data.reason)
            if (_this.lastElement)
              if (_this.currentPage !== 1)
                _this.currentPage -= 1
            axios.get('http://localhost:8181/api/sales/getsales/'+_this.currentPage,{params: _this.search}).then(function (resp) {
              if (resp.data.code === 200) {
                _this.sales = resp.data.data.content
                _this.total = resp.data.data.totalElements
                _this.lastElement = (resp.data.data.last && resp.data.data.numberOfElements === 1)
                ...
                axios.get('http://localhost:8181/api/sales/getsales/1',{params: _this.search}).then(function (resp) {
                  if (resp.data.code === 200) {
                    _this.sales = resp.data.data.content
                    _this.total = resp.data.data.totalElements
                    _this.currentPage = 1
    ...
    save(row, index) {
      const _this = this
      axios.post('http://localhost:8181/api/sales/savesales', row).then(function (resp) {
        _this.$message.success(resp.data.reason)
      })
    ...
    // 取消商品属性编辑
    cancel(row, index) {
      this.$set(this.sales[index], 'price',sessionStorage.getItem('oldPrice'))
      this.$set(this.sales[index], 'sumPrice',sessionStorage.getItem('oldSumPrice'))
